...
const makeRegistrationSocket = (config) => {
  ...
  const register = async (code) => {
    ...
    const result = await mobileRegister({ ...sock.authState.creds, ...sock.authState.creds.registration, code });
...
function mobileRegisterExists(params) {
 ...
 */
async function mobileRegister(params) {
